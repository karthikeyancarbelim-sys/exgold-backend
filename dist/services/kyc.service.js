...
exports.verifyNerotixPan = exports.getNerotixDigilockerData = exports.createKycRequest = void 0;
...
const nerotixHostedOrSession = async (user) => {
    const createSessionUrl = process.env.NEROTIX_CREATE_SESSION_URL ||
        "https://api.nerofy.in/api/v1/service/digilocker/verify";
    const response = await axios_1.default.post(createSessionUrl, {
...
const getNerotixDigilockerData = async (kyc) => {
    const url = process.env.NEROTIX_DIGILOCKER_GET_DATA_URL ||
        "https://api.nerofy.in/api/v1/service/digilocker/get-data";
    const refId = kyc.nerotix_reference_id ||
        kyc.reference_id ||
        kyc.aadhaar_reference_id ||
        kyc.video_reference_id;
    const txnId = kyc.aadhaar_txn_id ||
        kyc.nerotix_txn_id ||
        kyc.video_reference_id ||
        kyc.reference_id;
    if (!refId || !txnId)
    ...
    try {
        const response = await axios_1.default.post(url, body, { headers: nerotixHeaders() });
        ...
        const response = await axios_1.default.post(fallbackUrl, body, { headers: nerotixHeaders() });
        return response.data;
    }
};
exports.getNerotixDigilockerData = getNerotixDigilockerData;
const verifyNerotixPan = async (panNumber) => {
    const url = process.env.NEROTIX_PAN_VERIFY_URL ||
        "https://api.nerofy.in/api/v1/service/pancard/verify";
    const response = await axios_1.default.post(url, { panNumber }, { headers: nerotixHeaders() });
    return response.data;
};
exports.verifyNerotixPan = verifyNerotixPan;
const kycaidHostedForm = async (user) => {
    ...
    const response = await axios_1.default.post(`${KYCAID_BASE_URL}/forms/${formId}/urls`, body, {
        headers: {

...
exports.getMyGoldTransactions = exports.startSipInvestment = exports.sellGoldInvestment = exports.getGoldBalance = exports.getInvestmentRates = exports.getInvestmentStatus = void 0;
...
const gold_history_1 = require("../utils/gold-history");
...
const providerTxnId = (payload) => extractDeep(payload, ["txnId", "transactionId", "transaction_id", "id", "orderId", "order_id"]);
const isProviderSuccess = augmont_payout_1.augmontTransactionConfirmed;
const providerAmount = (payload) => Number(extractDeep(payload, ["amount", "totalAmount", "total_amount", "preTaxAmount"]) || 0);
const merchantTxnId = (prefix, userId) => `${prefix}_${userId}_${Date.now()}`;
const getUser = async (uid) => {
    const result = await db_1.pool.query("SELECT id, kyc_status, gold_balance FROM users WHERE firebase_uid=$1", [uid]);
    return result.rows[0];
};
const kycBlockStatus = (eligibility) => eligibility.code === "kyc_required" || eligibility.code === "kyc_expired" ? 403 : 409;
...
const getInvestmentSettings = async () => {
    ...
    return {
        ...
        minimumBuyAmount: augmont_merchant_policy_1.augmontMerchantPolicy.minimumBuyAmount,
        maximumBuyAmount: augmont_merchant_policy_1.augmontMerchantPolicy.maximumBuyAmount,
        maximumSellAmount: augmont_merchant_policy_1.augmontMerchantPolicy.maximumSellAmount,
        kycFinancialYearThreshold: augmont_merchant_policy_1.augmontMerchantPolicy.kycFinancialYearThreshold,
        sellLockHours: augmont_merchant_policy_1.augmontMerchantPolicy.sellLockHours,
        buyTaxRate: Number(process.env.AUGMONT_GOLD_TAX_RATE || 3),
    };
};
const financialYearBuyTotal = async (userId) => {
    const result = await db_1.pool.query(`SELECT COALESCE(SUM(amount),0)::numeric AS total
     FROM gold_transactions
     WHERE user_id=$1 AND type='buy'
       AND LOWER(COALESCE(status,'')) IN ('success','completed')
       AND COALESCE(settled_at, created_at) >= $2`, [userId, (0, augmont_merchant_policy_1.indiaFinancialYearStart)()]);
    return (0, augmont_merchant_policy_1.roundMoney)(Number(result.rows[0]?.total || 0));
};
const sellableGold = async (userId, availableGold, db = db_1.pool) => {
    const result = await db.query(`SELECT
     ...
     FROM gold_transactions WHERE user_id=$1`, [userId, augmont_merchant_policy_1.augmontMerchantPolicy.sellLockHours]);
    ...
    return {
        sellableGoldGrams: (0, augmont_merchant_policy_1.roundGold)(sellable),
        lockedGoldGrams: (0, augmont_merchant_policy_1.roundGold)(Math.max(availableGold - sellable, 0)),
        sellUnlockAt: nextLockedBuy
            ? new Date(nextLockedBuy.getTime() + augmont_merchant_policy_1.augmontMerchantPolicy.sellLockHours * 3600000).toISOString()
            : null,
    };
};
const getInvestmentStatus = async (req, res) => {
    try {
        ...
        const kycFreePurchaseRemaining = Math.max(settings.kycFinancialYearThreshold - fyBuyTotal, 0);
        ...
        const sellEnabled = providerSafety.safe && settings.goldSellEnabled && investmentKycApproved;
        return res.json({
            enabled: true,
            ...
            kycFreePurchaseRemaining: (0, augmont_merchant_policy_1.roundMoney)(kycFreePurchaseRemaining),
            ...settings,
            options: [
                "buy",
                "sell",
...
const getInvestmentRates = async (_req, res) => {
    try {
        const rates = await (0, augmont_service_1.augmontGetRates)({ allowStale: true });
...
exports.getInvestmentRates = getInvestmentRates;
const getGoldBalance = async (req, res) => {
    try {
        const user = await getUser(req.user.uid);
        if (!user)
            return res.status(404).json({ message: "User not found" });
        const totals = await db_1.pool.query(`SELECT
       ...
       FROM gold_transactions
       WHERE user_id=$1`, [user.id, confirmedStatuses]);
        ...
        try {
            const rates = await (0, augmont_service_1.augmontGetRates)({ allowStale: true });
            sellRate = Number(extractDeep(rates, ["gSell", "goldSell", "gold_sell", "sellRate", "sell_rate"]) || 0);
        }
        catch (error) {
        ...
        return res.json({
            ...
            valuationAvailable,
            unrealizedPnl: currentValue === null ? null : Number((currentValue - investedCost).toFixed(2)),
            provider: "augmont",
...
exports.getGoldBalance = getGoldBalance;
...
const sellGoldInvestment = async (req, res) => {
    ...
    try {
        ...
        const quantityError = (0, augmont_merchant_policy_1.validateSellQuantity)(grams);
        if (quantityError)
        ...
        if (grams > availability.sellableGoldGrams + 0.0000001) {
            return res.status(400).json({
                ...
                message: availability.lockedGoldGrams > 0
                    ? `Only ${availability.sellableGoldGrams.toFixed(4)}g is sellable. New purchases are locked for ${augmont_merchant_policy_1.augmontMerchantPolicy.sellLockHours} hours.`
                    : `Insufficient eligible gold. Available ${availability.sellableGoldGrams.toFixed(4)}g`,
                ...availability,
            });
        }
        const quote = await (0, augmont_service_1.augmontGetRates)({ allowStale: false });
        const liveSellRate = Number(extractDeep(quote, ["gSell", "goldSell", "gold_sell", "sellRate", "sell_rate"]) || 0);
        ...
        if ((0, augmont_merchant_policy_1.roundMoney)(grams * liveSellRate) > augmont_merchant_policy_1.augmontMerchantPolicy.maximumSellAmount) {
            return res.status(400).json({
                message: `Maximum gold sale is Rs.${augmont_merchant_policy_1.augmontMerchantPolicy.maximumSellAmount.toFixed(0)}`,
            });
        }
        const eligibility = await (0, investment_kyc_service_1.getInvestmentKycEligibility)(req.user.uid, {
            refreshProvider: true,
        ...
        const duplicate = await db_1.pool.query("SELECT * FROM gold_transactions WHERE merchant_txn_id=$1 LIMIT 1", [merchantTransactionId]);
        if (duplicate.rows.length) {
            const row = duplicate.rows[0];
            if (Number(row.user_id) !== localUserId)
        ...
        const reservation = await db_1.pool.connect();
        try {
            await reservation.query("BEGIN");
            const existing = await reservation.query("SELECT * FROM gold_transactions WHERE merchant_txn_id=$1 LIMIT 1 FOR UPDATE", [merchantTransactionId]);
            ...
            const lockedUser = await reservation.query("SELECT gold_balance FROM users WHERE id=$1 FOR UPDATE", [user.id]);
            ...
            const inserted = await reservation.query(`INSERT INTO gold_transactions
         ...
         RETURNING id`, [
            ...
            await reservation.query("UPDATE users SET gold_balance=gold_balance-$1 WHERE id=$2", [grams, user.id]);
            await reservation.query("COMMIT");
        ...
        const augmont = await (0, augmont_service_1.augmontSellGold)({
            userId: req.user.uid,
            grams,
            merchantTransactionId,
        });
        ...
        if (!providerSucceeded) {
            if ((0, augmont_payout_1.augmontSaleState)(augmont).state !== "failed") {
           ...
           WHERE id=$4 AND settled_at IS NULL`, [augmontTransactionId || null, (0, provider_payload_1.redactProviderPayload)(augmont), amount, localTransactionId]);
            ...
            const rejected = await db_1.pool.connect();
            try {
                await rejected.query("BEGIN");
                const row = await rejected.query(`SELECT status, settled_at, gold_restored_at
           FROM gold_transactions WHERE id=$1 FOR UPDATE`, [localTransactionId]);
                if (row.rows[0] && !row.rows[0].settled_at && !row.rows[0].gold_restored_at) {
                    await rejected.query(`UPDATE gold_transactions
             SET augmont_txn_id=$1, provider_payload=$2, amount=$3,
                 status='failed', payout_status='sell_rejected',
                 gold_restored_at=NOW()
             WHERE id=$4`, [
                        augmontTransactionId || null,
                        (0, provider_payload_1.redactProviderPayload)(augmont),
                        amount,
                        localTransactionId,
                    ]);
                    await rejected.query("UPDATE users SET gold_balance=COALESCE(gold_balance,0)+$1 WHERE id=$2", [grams, user.id]);
                }
        ...
        if (!Number.isFinite(amount) || amount <= 0) {
         ...
         WHERE id=$3 AND settled_at IS NULL`, [augmontTransactionId || null, (0, provider_payload_1.redactProviderPayload)(augmont), localTransactionId]);
            return res.status(202).json({
                success: false,
        ...
        const walletCredit = await db_1.pool.connect();
        try {
            await walletCredit.query("BEGIN");
            await (0, augmont_merchant_settlement_service_1.creditConfirmedMerchantSale)(walletCredit, {
                transactionId: Number(localTransactionId),
                userId: Number(user.id),
                amount,
                providerTransactionId: augmontTransactionId,
                providerPayload: (0, provider_payload_1.redactProviderPayload)(augmont),
            });
        ...
        const balance = await db_1.pool.query("SELECT gold_balance FROM users WHERE id=$1", [user.id]);
        ...
        if (reservationCreated && localTransactionId && localUserId) {
            ...
            const failure = await db_1.pool.connect();
            try {
                await failure.query("BEGIN");
                const row = await failure.query(`SELECT status, settled_at, gold_restored_at, gold_grams
           FROM gold_transactions WHERE id=$1 FOR UPDATE`, [localTransactionId]);
                if (row.rows[0] && !row.rows[0].settled_at) {
                    ...
                    if (definitiveRejection &&
                        ...
                        await failure.query("UPDATE users SET gold_balance=COALESCE(gold_balance,0)+$1 WHERE id=$2", [Number(row.rows[0].gold_grams || 0), localUserId]);
                    }
                    await failure.query(`UPDATE gold_transactions
...
exports.sellGoldInvestment = sellGoldInvestment;
const startSipInvestment = async (_req, res) => {
    return res.status(501).json({
        message: "Recurring gold purchases are not available yet",
    });
};
exports.startSipInvestment = startSipInvestment;
...
const getMyGoldTransactions = async (req, res) => {
    try {
        const user = await getUser(req.user.uid);
        if (user) {
            await (0, digital_gold_sale_settlement_service_1.reconcilePendingDigitalGoldSales)({
                userId: Number(user.id),
                limit: 5,
            }).catch(() => null);
        }
        const result = await db_1.pool.query(`WITH user_record AS (
       ...
       ORDER BY created_at DESC`, [req.user.uid]);
        const rows = result.rows.map(gold_history_1.goldHistoryDetails);
...
exports.getMyGoldTransactions = getMyGoldTransactions;

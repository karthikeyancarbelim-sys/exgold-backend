...
exports.getCategories = exports.activateFeaturedAd = exports.updateAdStatus = exports.deleteAd = exports.updateAd = exports.getMyAds = exports.getAdById = exports.getAds = exports.createAd = void 0;
...
const karatExpr = (alias) => `NULLIF(regexp_replace(${alias}.karat::text, '[^0-9]', '', 'g'), '')::int`;
const purityExpr = (alias) => `NULLIF(regexp_replace(${alias}.purity::text, '[^0-9]', '', 'g'), '')::int`;
const rowToFlutterAd = (row) => {
...
const isInternalTeam = (role) => ["internal", "team", "staff", "admin", "superadmin"].includes(role);
/* ===============================
   CREATE AD
   =============================== */
const createAd = async (req, res) => {
    try {
        const { title, description, grams, weight, purity, wastage, making_charges, makingCharge, city, price, categoryId, category_id, metal, condition, images, sellerName, status, } = req.body;
        ...
        const userResult = await db_1.pool.query(`SELECT id, name, city, role, profile_completed, subscription_active, subscription_end, trial_used,
       ...
       FROM users WHERE firebase_uid=$1`, [uid]);
        ...
        const rateResult = await db_1.pool.query("SELECT price_per_gram FROM gold_rates WHERE NULLIF(regexp_replace(karat::text, '[^0-9]', '', 'g'), '')::int=$1 ORDER BY updated_at DESC LIMIT 1", [adPurity]);
      ...
      RETURNING *`, [
        ...
        if (adStatus !== "active") {
            await db_1.pool.query("UPDATE classified_ads SET status=$1 WHERE id=$2", [
                adStatus,
                result.rows[0].id,
            ]);
...
const getAds = async (req, res) => {
    try {
        ...
        if (city)
            where.push(`LOWER(ca.city) = LOWER(${push(city)})`);
        if (categoryId)
            where.push(`ca.category_id = ${push(categoryId)}`);
        ...
        const [result, adsSnap] = await Promise.all([
            db_1.pool.query(`SELECT ca.id, ca.title, ca.description, ca.grams, ca.purity, ca.wastage,
        ...
        if (page === 1)
            adsSnap?.docs.forEach((doc) => {
                ...
                if (String(data.status || "active").toLowerCase() !== "active")
                    return;
                if (city && String(data.city || "").toLowerCase() !== city.toLowerCase())
                    return;
                if (categoryId && String(data.categoryId || data.category_id || "") !== categoryId)
                    return;
                if (condition && condition.toLowerCase() !== "all" && String(data.condition || "New").toLowerCase() !== condition.toLowerCase())
                    return;
                if (purity && purity.toLowerCase() !== "all" && parsePurity(data.purity) !== parsePurity(purity))
                    return;
...
const getAdById = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await db_1.pool.query(`SELECT ca.*, gr.price_per_gram AS current_gold_rate, u.firebase_uid, u.name, u.role
       FROM classified_ads ca
       LEFT JOIN users u ON u.id = ca.user_id
       LEFT JOIN gold_rates gr ON ${karatExpr("gr")} = ${purityExpr("ca")}
       WHERE ca.id=$1`, [id]);
...
const getMyAds = async (req, res) => {
    try {
       ...
       ORDER BY ca.created_at DESC`, [uid]);
...
const updateAd = async (req, res) => {
    try {
        const { id } = req.params;
        const uid = req.user.uid;
        const { title, description, grams, weight, purity, wastage, making_charges, makingCharge, city, price, status, categoryId, category_id, metal, condition, images, sellerName, } = req.body;
        const adWeight = Number(grams ?? weight);
        const adPurity = parsePurity(purity);
        const adMakingCharges = Number(making_charges ?? makingCharge ?? 0);
        if (!adWeight || adWeight <= 0) {
...
const deleteAd = async (req, res) => {
    try {
        const { id } = req.params;
        const uid = req.user.uid;
        await db_1.pool.query(`UPDATE classified_ads SET status='deleted'
       WHERE id=$1 
       AND user_id=(
...
const updateAdStatus = async (req, res) => {
    try {
        ...
        await db_1.pool.query(`UPDATE classified_ads
       SET status=$1
       WHERE id=$2 AND user_id=(SELECT id FROM users WHERE firebase_uid=$3)`, [status, id, req.user.uid]);
...
const activateFeaturedAd = async (req, res) => {
    ...
    try {
        ...
        const adResult = await client.query(`UPDATE classified_ads
       SET is_featured=true,
           featured_until=NOW() + ($1 || ' days')::interval
       WHERE id=$2 AND user_id=$3
       RETURNING *`, [days, id, userId]);
        if (adResult.rows.length === 0) {
            throw new Error("Ad not found or unauthorized");
        }
        await client.query("UPDATE wallets SET balance=balance-$1 WHERE user_id=$2", [cost, userId]);
        await client.query(`INSERT INTO wallet_transactions
       (user_id, type, amount, method, status)
       VALUES ($1, 'debit', $2, 'featured_ad', 'success')`, [userId, cost]);
...
exports.activateFeaturedAd = activateFeaturedAd;

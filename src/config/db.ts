...
pool.connect()
  .then(async (client) => {
    ...
    for (const karat of [18, 22, 24]) {
      const existingMargin = await pool.query(
        "SELECT karat FROM gold_margin WHERE karat=$1 LIMIT 1",
        [karat]
      );

      if (!existingMargin.rows.length) {
        await pool.query(
          "INSERT INTO gold_margin (karat, buy_margin, sell_margin) VALUES ($1, 0, 0)",
          [karat]
        );
      }
    }
    ...
    for (const [id, name, image, type, sortOrder] of defaultCategories) {
      ...
      if (!existingCategory.rows.length) {
        await pool.query(
          `INSERT INTO categories (id, name, image, type, sort_order)
           VALUES ($1, $2, $3, $4, $5)`,
          [id, name, image, type, sortOrder]
        );
      }
    }
    ...
    const existingExgoldUser = await pool.query(
      "SELECT id FROM users WHERE firebase_uid=$1 ORDER BY id LIMIT 1",
      ["internal_exgold"]
    );
    ...
    const exgoldUser = existingExgoldUser.rows.length
      ? await pool.query(
          `UPDATE users
           ...
           WHERE id=$1
           RETURNING id`,
          [existingExgoldUser.rows[0].id]
        )
      : await pool.query(
          `INSERT INTO users (firebase_uid, name, phone, role, city, profile_completed, ads_limit)
           VALUES ('internal_exgold','ExGold','','internal','India',true,999)
           RETURNING id`
        );

    const existingExgoldWallet = await pool.query(
    ...
    const adUserIdColumn = await pool.query(`
    ...
    const adUserIdType = String(
      adUserIdColumn.rows[0]?.udt_name || adUserIdColumn.rows[0]?.data_type || ""
    );
    const canSeedNumericAdUser =
      adUserIdType.includes("int") || adUserIdType === "numeric";

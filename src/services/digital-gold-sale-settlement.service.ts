...
import {
  ...
  augmontSaleState,
  augmontWithdrawalState,
} from "../utils/augmont-payout";
import {
  accountLast4,
  redactProviderPayload,
} from "../utils/provider-payload";
import { getAugmontMoneySafety } from "../utils/provider-environment";
import { creditConfirmedMerchantSale } from "./augmont-merchant-settlement.service";

type ReconcileSaleInput = {
...
const providerTransactionId = (payload: any) =>
  String(
    extractDeep(payload, ["transactionId", "transaction_id", "txnId", "txn_id", "id"]) ||
      ""
  );

const providerAmount = (payload: any) =>
  Number(extractDeep(payload, ["totalAmount", "total_amount", "amount", "preTaxAmount"]) || 0);

const loadSale = async (input: ReconcileSaleInput) => {
  const values: any[] = [];
...
const withdrawStatus = async (sale: any, providerId: string) => {
  ...
  for (const sellTransactionId of candidates) {
    try {
      return await augmontGetWithdrawStatus(sellTransactionId, sale.firebase_uid);
    } catch (error) {
      if (!isAugmontMissingResourceError(error)) throw error;
    }
  }
  return null;
};

const reconcileMerchantSale = async (sale: any) => {
  ...
  const sell = await augmontGetSellStatus(sale.merchant_txn_id, sale.firebase_uid);
  ...
  const providerSale = augmontSaleState(sell);
  const snapshot = {
    ...(sale.provider_payload || {}),
    sell: redactProviderPayload(sell),
    settlement: {
      route: "exgold_wallet",
  ...
  if (providerSale.state === "failed") {
    ...
    try {
      ...
      const locked = await client.query(
        `SELECT user_id, gold_grams, settled_at, gold_restored_at
      ...
      if (current && !current.settled_at && !current.gold_restored_at) {
        await client.query(
          ...
          [Number(current.gold_grams || 0), current.user_id]
        );
      }
  ...
  if (providerSale.state === "pending" || !Number.isFinite(amount) || amount <= 0) {
    await pool.query(
      ...
      [providerId, amount, snapshot, sale.id]
    );
    return {
      success: false,
  ...
  try {
    await client.query("BEGIN");
    const credit = await creditConfirmedMerchantSale(client, {
...
export const reconcileDigitalGoldSale = async (input: ReconcileSaleInput) => {
  const sale = await loadSale(input);
  if (!sale) throw new Error("Gold sale not found");
  if (String(sale.payout_route || "") === "exgold_wallet") {
    return reconcileMerchantSale(sale);
  }
  if (
    String(sale.payout_route || "") === "augmont_direct" &&
  ...
  if (sale.settled_at && String(sale.payout_status || "") === "paid") {
    return {
      ...
      destinationLast4: accountLast4(sale.account_number),
  ...
  const providerSale = augmontSaleState(sell);
  const sellDestination = augmontDestinationState(sell, destination);
  const baseSnapshot = {
    ...(sale.provider_payload || {}),
    sell: redactProviderPayload(sell),
    sale: {
        ...
        await client.query(
          `UPDATE gold_transactions
           SET augmont_txn_id=COALESCE(NULLIF($1,''),augmont_txn_id),
               ...
               provider_payload=$3
           WHERE id=$4`,
          [providerId, amount, baseSnapshot, sale.id]
        );
      }
      await client.query("COMMIT");
    } catch (error) {
  ...
  const payout = augmontWithdrawalState(withdrawal);
  const withdrawalDestination = augmontDestinationState(withdrawal, destination);
  ...
  const snapshot = {
    ...baseSnapshot,
    withdrawal: redactProviderPayload(withdrawal),
    payout: {
    ...
    await pool.query(
      `UPDATE gold_transactions
       SET augmont_txn_id=COALESCE(NULLIF($1,''),augmont_txn_id),
           ...
           provider_payload=$4
       WHERE id=$5 AND settled_at IS NULL`,
      [providerId, amount, payout.reference, snapshot, sale.id]
    );
    return {
      success: false,
      status: "payout_review",
  ...
  await pool.query(
    `UPDATE gold_transactions
     SET augmont_txn_id=COALESCE(NULLIF($1,''),augmont_txn_id),
         ...
         provider_payload=$6
     WHERE id=$7 AND settled_at IS NULL`,
    [providerId, amount, status, payout.status, payout.reference, snapshot, sale.id]
  );
  return {
    success: false,
    status,
...
export const reconcilePendingDigitalGoldSales = async ({
  userId,
  limit = 20,
}: {
  userId?: number;
  ...
  for (const row of rows.rows) {
    try {
      results.push(await reconcileDigitalGoldSale({ transactionId: Number(row.id) }));
    } catch (error: any) {
      results.push({
        success: false,
...
export const startDigitalGoldSaleReconciliation = () => {
  if (reconciliationTimer || !getAugmontMoneySafety().safe) return;
  const intervalMs = Math.max(
    Number(process.env.AUGMONT_SALE_RECONCILE_INTERVAL_MS || 120000),
    60000
  );
  const run = () =>
    reconcilePendingDigitalGoldSales().catch((error) =>
      console.error("AUGMONT SALE RECONCILIATION ERROR:", error?.message || error)
    );

...
const validityMs = 15 * 60_000;
const lockKey = 2_046_735_912;
let ready: Promise<void> | null = null;

const ensureStore = () => {
  if (!ready) {
    ready = pool.query(`CREATE TABLE IF NOT EXISTS provider_rate_cache (
...
export const sharedAugmontRates = async (
  ...
  try {
    ...
    const result = await client.query(
      "SELECT payload, fetched_at, attempted_at FROM provider_rate_cache WHERE provider='augmont'",
    );
    const row = result.rows[0];
    const now = Date.now();
    const age = row?.fetched_at ? now - new Date(row.fetched_at).getTime() : Infinity;
    if (row?.payload && age >= 0 && age < refreshMs) return row.payload;
    const attemptedAge = row ? now - new Date(row.attempted_at).getTime() : Infinity;
    if (attemptedAge < refreshMs) {
      if (allowStale && row?.payload && age >= 0 && age < validityMs) return row.payload;
    ...
    try {
      ...
      if (allowStale && row?.payload && staleAge >= 0 && staleAge < validityMs) return row.payload;
      throw error;
    }
  } finally {
    try {
      if (locked) await client.query("SELECT pg_advisory_unlock($1)", [lockKey]);
    } finally {
      client.release();
    }
  }

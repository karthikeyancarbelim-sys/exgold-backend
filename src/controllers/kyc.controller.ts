...
import {
  createKycRequest,
  getNerotixDigilockerData,
  KycStep,
  verifyNerotixPan,
} from "../services/kyc.service";
import { syncApprovedKycToAugmont } from "../services/investment-kyc.service";
import {
  extractMaskedAadhaar,
  extractNerotixAadhaarEvidence,
  selectNerotixAadhaarEvidence,
} from "../utils/kyc-evidence";

const steps: KycStep[] = ["aadhaar", "pan", "video", "full"];

const normalizeStep = (value: unknown): KycStep | null => {
  const step = String(value || "full").toLowerCase();
  return steps.includes(step as KycStep) ? (step as KycStep) : null;
};

const overallStatus = (kyc: any) => {
  if (!kyc) return "none";
...
const isApprovedStatus = (status: string) => ["full", "approved"].includes(status.toLowerCase());

const kycExpiryDate = (kyc: any) => {
  if (!kyc) return null;
  if (kyc.kyc_expires_at) return new Date(kyc.kyc_expires_at);
...
const findValueDeep = (input: any, keys: string[]): any => {
  ...
  for (const [key, value] of Object.entries(input)) {
    ...
    if (keys.includes(normalizedKey) && value) return value;
...
const publicKyc = (kyc: any) => {
  ...
  const aadhaarEvidence = selectNerotixAadhaarEvidence(aadhaar_payload, nerotix_payload);
...
const getOrCreateKycRow = async (userId: number) => {
  ...
  const created = await pool.query(
    `INSERT INTO kyc (user_id, status)
     VALUES ($1,'none')
     RETURNING *`,
    [userId]
  );
...
const textValueDeep = (input: any, keys: string[]) => {
  const value = findValueDeep(input, keys);
  return value === null || value === undefined ? "" : String(value).trim();
};

const syncUserKycStatus = async (userId: number, kyc: any) => {
    ...
    await pool.query(
      `UPDATE kyc
       SET status='full',
           ...
           updated_at=NOW()
       WHERE id=$1`,
      [kyc.id, kycValidityDays]
    );

    await syncApprovedKycToAugmont(userId, kyc);
  }

  return status;
};

const isNerotixSuccess = (payload: any) => {
  return extractNerotixAadhaarEvidence(payload).verified;
};

const nerotixFailureMessage = (payload: any) =>
  String(payload?.message || payload?.data?.message || "").trim().toLowerCase();

const isNerotixRejected = (payload: any) => {
  if (payload?.success !== false || Number(payload?.statusCode) !== 0) return false;
  const message = nerotixFailureMessage(payload);
  return (
    message.includes("invalid aadhaar") ||
...
const isNerotixSessionExpired = (payload: any) => {
  if (payload?.success !== false || Number(payload?.statusCode) !== 0) return false;
  const message = nerotixFailureMessage(payload);
  return (
...
const isKycProviderAuthFailure = (error: any) => {
...
const kycProviderUnavailable = (res: Response) =>
  res.status(503).json({
    success: false,
...
const refreshNerotixAadhaarStatus = async (kyc: any) => {
  const provider = String(kyc?.kyc_provider || "").toLowerCase();
  const aadhaarStatus = String(kyc?.aadhaar_status || "").toLowerCase();
  if (provider !== "nerotix" || aadhaarStatus !== "pending") return kyc;

  try {
    const payload = await getNerotixDigilockerData(kyc);
    if (!payload) return kyc;
    ...
    if (!approved && !rejected && !expired) return kyc;
    ...
    const aadhaarMasked = approved ? extractMaskedAadhaar(payload) : null;
    const result = await pool.query(
      `UPDATE kyc
       ...
       WHERE id=$3
       RETURNING *`,
      [nextStatus, payload, kyc.id, aadhaarMasked]
    );
    ...
    if (
      [400, 404, 410, 422].includes(providerHttpStatus) &&
...
const scheduleNerotixAadhaarRefresh = (kycId: number) => {
  const delays = [
    8000,
    20000,
    45000,
  ...
  delays.forEach((delay) => {
    setTimeout(async () => {
      try {
        const result = await pool.query("SELECT * FROM kyc WHERE id=$1 LIMIT 1", [kycId]);
        const kyc = result.rows[0];
        if (!kyc || String(kyc.aadhaar_status || "").toLowerCase() !== "pending") return;
...
export const startKyc = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!step) return res.status(400).json({ message: "Invalid KYC step" });
    ...
    const kycRequest = await createKycRequest(user);
    const referenceId =
      kycRequest.id ||
    ...
    const updated = isKycaid
      ? await pool.query(
          `UPDATE kyc
           ...
           WHERE id=$3
           RETURNING *`,
          [referenceId, kycRequest.form_url || kycRequest.url || kycRequest.formUrl || null, row.id, provider]
        )
      : await pool.query(
          `UPDATE kyc
...
export const createKycSession = async (req: AuthRequest, res: Response) => {
  try {
    const userResult = await pool.query(
      "SELECT id, name FROM users WHERE firebase_uid=$1",
      [req.user.uid]
    ...
    const updatedKyc = await pool.query(
      `UPDATE kyc
       SET aadhaar_status='pending',
           status='PENDING',
...
export const verifyPan = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const payload = await verifyNerotixPan(panNumber);
...
export const kycaidCallback = async (req: Request, res: Response) => {
  try {
    ...
    const providedSecret = String(
      req.query.token || req.query.secret || req.headers["x-kycaid-secret"] || ""
    ).trim();
    ...
    const externalApplicantId =
      payload.applicant?.external_applicant_id ||
    ...
    const legacyKycStatus =
      status === "APPROVED" ? "full" : status === "REJECTED" ? "rejected" : "pending";
    ...
    await pool.query(
      `UPDATE kyc
       SET aadhaar_status=$1,
           ...
           updated_at=NOW()
       WHERE user_id=$6`,
      [stepStatus, status, applicantId, verificationId, payload, userId, kycValidityDays]
    );
    ...
    await pool.query(
      `UPDATE users
       ...
       WHERE id=$5`,
      [legacyKycStatus, status === "APPROVED", applicantId, verificationId, userId]
    );
...
export const nerotixCallback = async (req: Request, res: Response) => {
  try {
    const payload = req.body || {};
    ...
    const matched = await pool.query(
      `SELECT *
       ...
       ORDER BY
         ...
         created_at DESC
       LIMIT 1`,
      [referenceId ? String(referenceId) : null, userId]
    );
    ...
    const callbackStored = await pool.query(
      `UPDATE kyc
    ...
    res.json(
      kyc
        ? {
            ...publicKyc(kyc),
            overallStatus:
              isKycExpired(kyc) && isApprovedStatus(overallStatus(kyc)) ? "expired" : overallStatus(kyc),

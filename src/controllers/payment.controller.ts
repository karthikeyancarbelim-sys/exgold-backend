...
type CheckoutPurpose = "digital_gold" | "physical_product";
...
const kycBlockStatus = (eligibility: InvestmentKycEligibility) => {
  if (eligibility.code === "user_not_found") return 404;
  if (eligibility.code === "kyc_required" || eligibility.code === "kyc_expired") return 403;
  return 409;
};
...
const paymentSignatureIsValid = (orderId: string, paymentId: string, signature: string) =>
  crypto
    .createHmac("sha256", process.env.RAZORPAY_KEY_SECRET!)
    .update(`${orderId}|${paymentId}`)
    .digest("hex") === signature;

const productPayable = (payload: any) =>
  Number(
    extractDeep(payload, ["sellingPrice", "productPrice", "price", "totalPrice", "mrp"]) || 0
  );

const getCurrentUser = async (uid: string) => {
  const result = await pool.query("SELECT id FROM users WHERE firebase_uid=$1 LIMIT 1", [uid]);
...
const createCheckoutIntent = async (
  ...
  await pool.query(
    ...
    [intentId, user.id, purpose, amount, order.id, metadata]
  );
...
export const createDigitalGoldCheckout = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (amount < augmontMerchantPolicy.minimumBuyAmount) {
      return res.status(400).json({
        message: `Minimum digital gold purchase is Rs.${augmontMerchantPolicy.minimumBuyAmount.toFixed(0)}`,
      });
    }
    if (amount > augmontMerchantPolicy.maximumBuyAmount) {
      return res.status(400).json({
        message: `Maximum digital gold purchase is Rs.${augmontMerchantPolicy.maximumBuyAmount.toFixed(0)}`,
      });
    }
    ...
    let eligibility: InvestmentKycEligibility | null = null;
    if (requiresKyc) {
    ...
    const checkout = await createCheckoutIntent(req, "digital_gold", amount, {
      merchantTransactionId: `EXGBUY_${crypto.randomBytes(10).toString("hex")}`,
      metalType: "gold",
...
export const createPhysicalProductCheckout = async (req: AuthRequest, res: Response) => {
  try {
    const providerSafety = getAugmontMoneySafety();
    if (!providerSafety.safe) {
      return res.status(503).json({
        ...
        code: providerSafety.code,
        message:
          "Product checkout is temporarily paused while the live fulfilment provider is configured. No payment was created.",
      });
    }
    ...
    const verifiedItems: Array<{ sku: string; quantity: number; unitAmount: number }> = [];
    for (const rawItem of items) {
      ...
      const product = await augmontGetProduct(sku);
...
export const verifyCheckoutPayment = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const providerPayment = (await razorpay.payments.fetch(paymentId)) as any;
    ...
    if (!providerSafety.safe) {
      await pool.query(
        ...
        [paymentId, providerSafety.message, checkoutId]
      );
      return res.status(202).json({
        success: false,
        status: "provider_pending",
        ...
        checkoutId,
        message:
          "Payment is confirmed and protected. Fulfilment is paused until the live provider is configured.",
      });
    }
    ...
    if (checkoutId) {
      await pool.query(
        "UPDATE checkout_intents SET status='provider_pending', error_message=$1, updated_at=NOW() WHERE id=$2",
        [error.message || "Provider processing failed", checkoutId]
      ).catch(() => null);
    }
...
export const verifyPayment = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const providerOrder = (await razorpay.orders.fetch(orderId)) as any;
    const providerPayment = (await razorpay.payments.fetch(paymentId)) as any;
    if (
      providerOrder.notes?.firebase_uid !== uid ||
...
export const verifySubscription = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const existingSubscriptionPayment = await client.query(
    ...
    await client.query(
      `UPDATE users
       SET subscription_active=true,
           ...
           ads_limit=999
       WHERE id=$2`,
      [durationDays, userId, plan_type || "MONTHLY"]
    );

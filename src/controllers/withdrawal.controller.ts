...
import { augmontMerchantPolicy } from "../utils/augmont-merchant-policy";
import {
  compareAccountHolderName,
  verifyBankAccountWithNerotix,
} from "../services/nerotixBank.service";
import { getInvestmentKycEligibility } from "../services/investment-kyc.service";
...
const createAuditLog = async (
  ...
  await pool
    .query(
      `INSERT INTO admin_audit_logs (action, entity_type, entity_id, metadata)
       VALUES ($1,$2,$3,$4)`,
...
const providerErrorMessage = (error: any) =>
  String(
...
const getExpectedAccountName = async (userId: number) => {
  const result = await pool.query(
    `SELECT u.name, latest_kyc.pan_name, latest_kyc.pan_status
     FROM users u
     LEFT JOIN LATERAL (
  ...
  const panApproved =
    String(row.pan_status || "").toLowerCase() === "approved" &&
...
const publicAccount = (row: any, includeFullNumber = false) => {
  if (!row) return null;
...
const publicWithdrawalRequest = (row: any) => {
  if (!row) return null;
  const status = String(row.status || "pending").toLowerCase();
  const payoutReference = String(
    row.processed_reference || row.provider_reference || ""
  ).trim();
  const payoutCompleted =
    status === "paid" && Boolean(payoutReference) && Boolean(row.completed_at);
...
const verifyAndPersistBankAccount = async (
  accountId: number,
  userId: number
) => {
  const accountResult = await pool.query(
    `SELECT id, account_number, ifsc, bank_name
  ...
  if (!account) {
    ...
    (error as any).code = "WITHDRAWAL_ACCOUNT_NOT_FOUND";
    throw error;
  }

  try {
    const verification = await verifyBankAccountWithNerotix(
      String(account.account_number || ""),
      String(account.ifsc || "")
    );
    const expectedName = await getExpectedAccountName(userId);
    const nameMatchStatus = compareAccountHolderName(
      expectedName.name,
      String(verification.accountHolderName || "")
    );
    const verified =
      verification.accountValid && nameMatchStatus === "matched";
...
export const getPayoutCapabilities = async (_req: Request, res: Response) =>
  res.json({
    bankVerification: {
      provider: "nerotix",
...
export const getWithdrawalAccount = async (
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT id, account_holder_name, bank_name, account_number, ifsc,
...
export const upsertWithdrawalAccount = async (
  req: AuthRequest,
  res: Response
) => {
  try {
    ...
    const preferredMethod = String(
      req.body.preferredMethod || req.body.preferred_method || "bank"
    ).toLowerCase();
    ...
    const accountDb = await pool.connect();
    let saved: any;
    ...
    const sameAccount = await accountDb.query(
      `SELECT id, ifsc, modification_count
       FROM withdrawal_accounts
    ...
    if (!sameAccount.rows.length) {
      const ownedByOther = await accountDb.query(
    ...
    const accountCount = await accountDb.query(
      "SELECT COUNT(*)::int AS count FROM withdrawal_accounts WHERE user_id=$1",
      [userId]
    );
    const existing = sameAccount.rows[0];
    if (!existing && Number(accountCount.rows[0]?.count || 0) >= augmontMerchantPolicy.maximumBankIds) {
      return res.status(409).json({
        code: "BANK_ID_LIMIT_REACHED",
        message: `A maximum of ${augmontMerchantPolicy.maximumBankIds} bank accounts can be registered.`,
      });
    }
    if (existing && Number(existing.modification_count || 0) >= augmontMerchantPolicy.maximumBankModifications) {
      return res.status(409).json({
        code: "BANK_MODIFICATION_LIMIT_REACHED",
        message: `This bank account has reached its ${augmontMerchantPolicy.maximumBankModifications}-modification limit.`,
      });
    }
    ...
    saved = existing
      ...
      : await accountDb.query(
          `INSERT INTO withdrawal_accounts
           ...
           RETURNING *`,
          [userId, accountHolderName, bankName, accountNumber, ifsc]
        );
    await accountDb.query("COMMIT");
    accountCommitted = true;
    } finally {
      try {
        if (!accountCommitted) await accountDb.query("ROLLBACK");
      } finally {
        accountDb.release();
      }
    }

    const verification = await verifyAndPersistBankAccount(
      saved.rows[0].id,
      userId
    );
    return res.json({
...
export const reverifyWithdrawalAccount = async (
  req: Request,
  res: Response
) => {
  try {
    const account = await pool.query(
      "SELECT id, user_id FROM withdrawal_accounts WHERE id=$1 LIMIT 1",
      [req.params.id]
    );
    ...
    await createAuditLog(
      "withdrawal_account_provider_recheck",
      "withdrawal_account",
...
const getWithdrawalSettings = async () => {
  const result = await pool.query(
    "SELECT value FROM app_settings WHERE key='withdrawal'"
  );
...
export const createWithdrawalRequest = async (
  ...
  try {
    ...
    const account = await pool.query(
      `SELECT id, bank_verified, bank_verification_status, status
       FROM withdrawal_accounts
       WHERE user_id=$1
       ORDER BY updated_at DESC
       LIMIT 1`,
    ...
    if (
      withdrawalAccount.bank_verified !== true ||
    ...
    const suppliedIdempotencyKey = String(
      req.body.clientRequestId || req.body.idempotencyKey || ""
    ).trim();
    if (
    ...
    const request = await client.query(
      `INSERT INTO withdrawal_requests
       ...
       RETURNING *`,
      [userId, withdrawalAccount.id, amount, idempotencyKey]
    );
    ...
    await client.query(
      `INSERT INTO wallet_transactions
       ...
       VALUES ($1,'hold',$2,'withdrawal_bank',
               'Withdrawal funds reserved','pending',$3)`,
    ...
    return res.status(201).json({
      ...
      message:
        "Withdrawal request submitted. No bank transfer has occurred; funds are reserved until a payout is completed and its reference is recorded.",
    });
  } catch (error) {
...
export const getMyWithdrawalRequests = async (
  ...
  try {
    ...
    if (!userId) return res.status(404).json({ message: "User not found" });
    const result = await pool.query(
      `SELECT id, amount, method, status, provider, provider_status,
...
export const getAllWithdrawalAccounts = async (
  ...
  try {
    const result = await pool.query(
      `SELECT wa.id, wa.user_id, wa.account_holder_name, wa.bank_name,
...
export const updateWithdrawalAccountStatus = async (
  req: Request,
  res: Response
) => {
  try {
    if (
      req.body.bankVerified === true ||
    ...
    const reason = String(
      req.body.reason || req.body.adminNotes || req.body.admin_notes || ""
    ).trim();
    const result = await pool.query(
      `UPDATE withdrawal_accounts
...
export const getAllWithdrawalRequests = async (
  ...
  try {
    const result = await pool.query(
      `SELECT wr.id, wr.user_id, wr.withdrawal_account_id, wr.amount,
...
export const updateWithdrawalRequestStatus = async (
  ...
  try {
    ...
    const payoutReference =
      requestedReference ||
      String(
        current.provider_reference || current.processed_reference || ""
      ).trim();
    ...
    if (status === "paid" && currentStatus !== "paid") {
      ...
      if (hold.rows.length) {
        const wallet = await client.query(
          `SELECT COALESCE(reserved_balance,0) AS reserved_balance
           FROM wallets WHERE user_id=$1 FOR UPDATE`,
          [current.user_id]
        );
        ...
        const legacyDebit = await client.query(
        ...
        if (!legacyDebit.rowCount) {
          const settledLegacyDebit = await client.query(
            `SELECT id FROM wallet_transactions
          ...
          if (!settledLegacyDebit.rows.length) {
            ...
            return res.status(409).json({
              message:
                "No matching wallet hold or debit exists. Reconcile before payout.",
            });
          }
        }
      }
    ...
    if (
      ...
      if (hold.rows.length) {
        ...
        if (Number(wallet.rows[0]?.reserved_balance || 0) < Number(current.amount)) {
          ...
          return res.status(409).json({
            message:
              "Reserved wallet amount is inconsistent. Reconcile before releasing funds.",
          });
        }
        await client.query(
          `UPDATE wallets
        ...
        if (legacyDebit.rowCount) {
          ...
          const refund = await client.query(
            `INSERT INTO wallet_transactions
             ...
             RETURNING id`,
            [current.user_id, current.amount, refundReference]
          );
          if (refund.rowCount) {
            await client.query(
    ...
    const result = await client.query(
      `UPDATE withdrawal_requests
       ...
       WHERE id=$5
       RETURNING *`,
      [status, adminNotes, payoutReference || null, providerStatus, req.params.id]
    );
    await client.query("COMMIT");

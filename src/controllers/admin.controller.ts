...
import { selectNerotixAadhaarEvidence } from "../utils/kyc-evidence";
import { settleDigitalGoldCheckout } from "../services/digital-gold-settlement.service";
import { reconcileDigitalGoldSale } from "../services/digital-gold-sale-settlement.service";
import { settlementAmountResult } from "../services/augmont-merchant-settlement.service";

const kycValidityDays = Number(process.env.KYC_VALIDITY_DAYS || 365);

const parseKarat = (karat: unknown) => {
  const value = Number(String(karat).replace(/k/i, ""));
  return [18, 22, 24].includes(value) ? value : null;
};

const kycOverallStatus = (kyc: any) => {
  const aadhaar = String(kyc.aadhaar_status || "none").toLowerCase();
  const pan = String(kyc.pan_status || "none").toLowerCase();

  if (String(kyc.status || "").toLowerCase() === "expired") return "expired";
  if (aadhaar === "rejected" || pan === "rejected") return "rejected";
  if (aadhaar === "approved" && pan === "approved") return "full";
...
const maskPan = (pan?: string | null) => {
  ...
  if (value.length < 10) return value || null;
...
const createAdminAuditLog = async (
  ...
  await pool.query(
    `INSERT INTO admin_audit_logs (action, entity_type, entity_id, actor, metadata)
     VALUES ($1,$2,$3,$4,$5)`,
    [
      action,
      entityType,
...
const safeScalar = async (sql: string, fallback = 0) => {
  try {
...
const userWhereClause = (id: string, offset: number) => {
...
const actorIsSuperAdmin = (req: Request) =>
  String((req as any).user?.role || "").toLowerCase() === "superadmin";

const rejectElevatedRoleChange = (req: Request, res: Response, role: unknown) => {
...
export const createAdminAccount = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [username || null, email, passwordHash, role]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE admins
       ...
       RETURNING id, username, email, role, status, last_login AS "lastLogin", created_at AS "createdAt"`,
      values
    );
...
export const deleteAdminAccount = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      "UPDATE admins SET status='blocked' WHERE id=$1 RETURNING id",
      [targetId]
    );
...
export const getDashboardStats = async (_req: Request, res: Response) => {
  try {
    const [usersSnap, adsSnap] = await Promise.all([
      admin.firestore().collection("users").get().catch(() => null),
...
export const getMonitoringOverview = async (_req: Request, res: Response) => {
  try {
    const [
      users,
      activeAds,
    ...
    return res.json({
      ...
      services: {
        database: "online",
        firebase: admin.apps.length ? "configured" : "not_configured",
...
export const getAllUsers = async (_req: Request, res: Response) => {
  try {
    ...
    usersSnap?.docs.forEach((doc) => {
      if (usersByUid.has(doc.id)) return;
...
export const blockUser = async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE users SET is_blocked=$1 WHERE ${where.clause} RETURNING id`,
      [is_blocked, where.value]
    );
...
export const createAdminUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (rejectElevatedRoleChange(req, res, role)) return;
    ...
    const result = await client.query(
      `INSERT INTO users
       ...
       RETURNING *`,
      [uid, name, phone, email, role, city, Boolean(name || phone || email), adsLimit]
    );
...
export const updateAdminUser = async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.role !== undefined && rejectElevatedRoleChange(req, res, req.body.role)) return;
...
export const deleteAdminUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await client.query(
      `SELECT id FROM users WHERE ${lookup.clause} LIMIT 1`,
      [lookup.value]
    );
...
export const getAllGoldTransactions = async (req: Request, res: Response) => {
  try {
...
export const confirmAugmontMerchantSettlement = async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const reference = String(
      req.body.settlementReference || req.body.settlement_reference || ""
    ).trim();
    const settledAmount = Number(
      req.body.settledAmount ?? req.body.settled_amount
    );
    ...
    const existing = await client.query(
      `SELECT * FROM augmont_merchant_settlements WHERE id=$1 FOR UPDATE`,
      [req.params.id]
    );
    ...
    const { variance, status } = settlementAmountResult(expectedAmount, settledAmount);
    const result = await client.query(
      `UPDATE augmont_merchant_settlements
       SET status=$1,
       ...
       WHERE id=$5
       RETURNING *`,
      [status, reference, settledAmount, notes, req.params.id]
    );
    await client.query("COMMIT");

    await createAdminAuditLog(
      "augmont_merchant_settlement_confirmed",
      "augmont_merchant_settlement",
...
export const reconcileDigitalGoldCheckout = async (req: Request, res: Response) => {
  try {
    const settlement = await settleDigitalGoldCheckout({
...
export const reconcileDigitalGoldSalePayout = async (req: Request, res: Response) => {
  try {
    const result = await reconcileDigitalGoldSale({
...
/* ===============================
   ...
   =============================== */
export const createAdminAd = async (req: Request, res: Response) => {
  try {
    ...
    const rateResult = await pool.query(
      "SELECT price_per_gram FROM gold_rates WHERE regexp_replace(karat::text, '[^0-9]', '', 'g')::int=$1 LIMIT 1",
      [purity]
    );
...
/* ===============================
   ...
   =============================== */
export const updateAdminAd = async (req: Request, res: Response) => {
  try {
    const allowedFields: Record<string, string> = {
      title: "title",
    ...
    Object.entries(allowedFields).forEach(([bodyKey, column]) => {
      ...
      if (bodyKey === "purity") value = parseKarat(value) || 22;
      values.push(value);
      updates.push(`${column}=$${values.length}`);
    });
...
/* ===============================
   ...
   =============================== */
export const deleteAdminAd = async (req: Request, res: Response) => {
  try {
    const result = await pool.query(
      "UPDATE classified_ads SET status='deleted' WHERE id=$1 RETURNING *",
      [req.params.id]
    );
...
export const approveAd = async (req: Request, res: Response) => {
  try {
    ...
    const adRef = admin.firestore().collection("ads").doc(id);
  ...
  try {
    ...
    return res.json(
      result.rows.map((row) => {
        ...
        const evidence = selectNerotixAadhaarEvidence(aadhaar_payload, nerotix_payload);
...
export const updateKycStatus = async (req: Request, res: Response) => {
  try {
    ...
    const stepColumns: Record<string, string> = {
    ...
    await pool.query(
      `UPDATE users
       ...
       WHERE id = $3`,
      [userKycStatus, kycVerified, kyc.user_id]
    );
    ...
    await pool.query(
      `UPDATE kyc
       SET status = $1,
           ...
           updated_at = NOW()
       WHERE id = $2`,
      [userKycStatus, kyc.id, kycVerified, kycValidityDays]
    );
...
/* ===============================
   ...
   =============================== */
export const forceKycReverify = async (req: Request, res: Response) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const existing = await client.query(
      `SELECT * FROM kyc WHERE id=$1 FOR UPDATE`,
      [id]
    );
    ...
    const result = await client.query(
      `UPDATE kyc
       ...
       WHERE id=$1
       RETURNING *`,
      [id, resetAadhaar, resetPan, reason]
    );
    ...
    const statusResult = await client.query(
      `UPDATE kyc SET status=$2, updated_at=NOW() WHERE id=$1 RETURNING *`,
      [id, nextStatus]
    );
...
/* ===============================
   ...
   =============================== */
export const getAdminGoldRates = async (_req: Request, res: Response) => {
  try {
    ...
    const rates = result.rows.reduce(
      (acc, row) => ({
        ...acc,
...
/* ===============================
   ...
   =============================== */
export const updateAdminGoldRates = async (req: Request, res: Response) => {
  const client = await pool.connect();
  ...
  try {
      ...
      if (updated.rowCount === 0) {
        await client.query(
          `INSERT INTO gold_rates (karat, price_per_gram, updated_at)
           VALUES ($1, $2, NOW())`,
          [rate.karat, rate.price]
        );
      }

      await client.query(
        `INSERT INTO gold_rate_history (karat, price_per_gram, source)
         VALUES ($1,$2,'admin')`,
        [rate.karat, rate.price]
      );
    }
    ...
    admin.firestore().collection("gold_rates").doc("today").set(
      {
...
/* ===============================
   ...
   =============================== */
export const updateGoldMargin = async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO gold_margin (karat, buy_margin, sell_margin, updated_at)
       VALUES ($3, $1, $2, NOW())
       ON CONFLICT (karat)
